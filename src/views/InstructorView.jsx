...
import { LogOut, Users, Award, FileText, ArrowLeft, RefreshCw, Loader2, AlertCircle } from 'lucide-react';
import { listStudentsRemote } from '../lib/storage';
...
export default function InstructorView({ modules, onLogout, onBackToAdmin }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setError('');
    try {
      const list = await listStudentsRemote();
      setStudents(list);
      setLastRefresh(new Date());
    } catch (e) {
      setError('Could not load student data: ' + e.message);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const getProgress = (s) => s.progress || {};
  const getSubmissions = (s) => s.submitted || {};

  const avgCompletion = students.length
    ? Math.round(students.reduce((sum, s) => {
        const p = getProgress(s);
        return sum + (Object.values(p).filter(Boolean).length / (modules.length || 1) * 100);
      }, 0) / students.length)
    : 0;
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button onClick={refresh} disabled={loading}
              className="flex items-center gap-1 text-sm text-slate-600 hover:text-slate-900 border border-slate-200 rounded px-2.5 py-1.5 hover:bg-slate-50 disabled:opacity-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {lastRefresh && (
          <p className="text-xs text-slate-400 mb-4 font-mono">
            Last refreshed {lastRefresh.toLocaleTimeString()} · {students.length} student{students.length === 1 ? '' : 's'}
          </p>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-xl p-6 border border-slate-200">
            ...
            <p className="text-3xl font-bold text-slate-900">{loading ? '—' : students.length}</p>
          </div>
          <div className="bg-white rounded-xl p-6 border border-slate-200">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-3xl font-bold text-violet-600">{loading ? '—' : avgCompletion}%</p>
          </div>
          <div className="bg-white rounded-xl p-6 border border-slate-200">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-3xl font-bold text-emerald-600">{loading ? '—' : totalSubs}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          {loading ? (
            <div className="px-6 py-12 text-center text-slate-500">
              <Loader2 size={28} className="animate-spin mx-auto mb-3 text-violet-400" />
              <p className="text-sm">Loading students from GitHub...</p>
            </div>
          ) : students.length === 0 ? (
            <div className="px-6 py-12 text-center text-slate-500">
              <Users size={36} className="mx-auto mb-3 opacity-40" />
              <p>No students yet. Share the app link with your cohort!</p>
              <p className="text-xs text-slate-400 mt-1">Students appear here once they sign in and mark progress.</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {students.map((s, idx) => {
                    ...
                    const key = s.email || s.name || idx;
                    return (
                      <tr key={key} className="border-b border-slate-200 hover:bg-slate-50">
                        <td className="px-6 py-4 text-sm font-medium text-slate-900">
                          {s.name}
                          {s.email && <span className="block text-xs text-slate-400 font-normal">{s.email}</span>}
                        </td>
                        {modules.map(m => (
